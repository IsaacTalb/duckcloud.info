...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      const e = error as Error & { code?: string }; const cmsError = error instanceof CmsError, timeout = e.name === 'TimeoutError' || e.name === 'AbortError';
      return json({ success: false, error: { code: cmsError ? error.code : timeout ? 'TIMEOUT' : e.code ?? 'REQUEST_FAILED', message: cmsError ? error.message : timeout ? 'The remote server did not respond within the timeout.' : e.code ? e.message : 'The request could not be completed.' }, meta: { requestId, durationMs: Date.now() - started } }, cmsError ? error.status : timeout ? 504 : 400, request, env);
    }
  },
};
